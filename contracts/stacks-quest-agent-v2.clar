...
(define-constant FEE-TREASURY-PCT u50)
(define-constant FEE-POOL-PCT     u50)
...
(define-map checkins
  ...
  { checked-in: bool, timestamp: uint, fee-paid: uint })
...
(define-public (daily-checkin)
  (let (
    ...
    ;; Collect fee - send to contract
    (try! (stx-transfer? CHECKIN-FEE tx-sender (as-contract tx-sender)))
    ...
    (let (
      ...
      (ok { streak: new-streak, bonus: bonus, day: today, fee: CHECKIN-FEE }))))
...
(define-public (claim-streak-reward)
  (let (
    ...
    (let ((payout (if (>= pool amount) amount pool)))
      (var-set reward-pool (- pool payout))

      (map-set streaks { user: user }
        (merge streak {
          pending-reward: u0,
          claimed-total:  (+ (get claimed-total streak) payout),
        }))

      (try! (stx-transfer? payout (as-contract tx-sender) user))
      (ok payout))))
