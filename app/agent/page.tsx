...
import { useWallet }
import { SwapCard } from '../components/SwapCard' from '../../hooks/useWallet'
...
function ActionCard({ action }: { action: any }) {
  ...
  if (action.type === 'bridge') {
    return (
      <div style={{ marginTop: 10, padding: '14px', borderRadius: 12, background: 'rgba(255,165,0,0.05)', border: '1px solid rgba(255,165,0,0.25)' }}>
        <div style={{ fontSize: 9, color: 'rgba(255,165,0,0.6)', letterSpacing: '0.2em', marginBottom: 8 }}>BRIDGE // {action.fromChain?.toUpperCase()} → {action.toChain?.toUpperCase()}</div>
        <div style={{ fontSize: 16, fontWeight: 700, color: '#ffaa00', marginBottom: 12 }}>{action.amount} {action.token}</div>
        <a href={BRIDGE_URL} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '10px 18px', borderRadius: 8, fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', background: '#ffaa00', color: 'black', textDecoration: 'none', fontFamily: 'inherit' }}>▶ OPEN BRIDGE ↗</a>
      </div>
    )
  }
  if (action.type === 'checkin') {
    return (
      <div style={{ marginTop: 10, padding: '10px 14px', borderRadius: 10, background: 'rgba(255,215,0,0.05)', border: '1px solid rgba(255,215,0,0.2)', fontSize: 10, color: '#ffd700', letterSpacing: '0.15em' }}>
...
async function callContract(
  ...
  try {
    ...
    console.error('[callContract]', e?.message || e)
    onCancel()
  }
}
...
export default function AgentPage() {
  ...
  const send = async () => {
    ...
    try {
      const res  = await fetch('/api/agent', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ messages: [...msgs, userMsg].map(m => ({ role: m.role, content: m.content })), address }) })
      const data = await res.json()
  ...
  const doCheckin = async () => {
    ...
    await callContract(
      ...
      () => { setMsgs(m => [...m, { role: 'assistant', content: '⚠ Check-in cancelled' }]); setChecking(false) }
    )
  }
  ...
  const doWithdraw = async () => {
    ...
    const stacks = await import('@stacks/transactions')
    await callContract(
      AGENT_V3, 'withdraw-treasury', [stacks.uintCV(Math.round(amt * 1_000_000))],
      (txid) => { setWithdrawTx(txid); setWithdrawAmt(''); setWithdrawing(false) },
      () => { setWithdrawErr('Cancelled or failed'); setWithdrawing(false) }
    )
  }
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          <div style={{ flex: 1, padding: '16px', overflowY: 'auto', paddingBottom: 80 }}>
            {msgs.length === 0 && (
              <div style={{ padding: '20px', borderRadius: 14, background: 'rgba(0,255,159,0.03)', border: '1px solid rgba(0,255,159,0.1)', marginBottom: 16 }}>
                ...
                <p style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', lineHeight: 1.6, marginBottom: 14 }}>Your intelligent crypto assistant on Stacks Bitcoin L2. I guide you through swaps, bridges, staking, and DeFi — you always keep control of your funds.</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {['What is my portfolio?', 'Swap 10 STX for $B2S', 'How do I earn yield?', 'Bridge from Base', 'How do streaks work?', 'What is sBTC?'].map(s => (
                    <button key={s} onClick={() => setInput(s)} style={{ fontSize: 9, padding: '5px 9px', borderRadius: 6, letterSpacing: '0.08em', fontFamily: 'inherit', cursor: 'pointer', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.4)' }}>{s}</button>
                  ))}
                </div>
              </div>
            )}
          ...
          <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, padding: '10px 12px', background: 'rgba(5,5,8,0.97)', borderTop: '1px solid rgba(255,255,255,0.06)', zIndex: 60 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              {!isConnected && <button onClick={connect} style={{ padding: '11px 14px', borderRadius: 9, fontSize: 9, fontWeight: 700, letterSpacing: '0.15em', fontFamily: 'inherit', cursor: 'pointer', background: 'white', border: 'none', color: 'black', whiteSpace: 'nowrap' }}>CONNECT</button>}
      ...
      {tab === 'portfolio' && (
        <div style={{ flex: 1, padding: '16px', overflowY: 'auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
            <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', letterSpacing: '0.25em' }}>{isConnected ? short(address!) : 'NOT CONNECTED'}</span>
            <button onClick={fetchPortfolio} style={{ fontSize: 9, padding: '4px 10px', borderRadius: 6, fontFamily: 'inherit', cursor: 'pointer', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.3)' }}>↻ REFRESH</button>
          </div>
          {!isConnected ? (
            ...
            <>
              <div style={{ padding: '20px', borderRadius: 14, background: 'rgba(0,255,159,0.04)', border: '1px solid rgba(0,255,159,0.12)', marginBottom: 14, textAlign: 'center' }}>
                <div style={{ fontSize: 9, color: 'rgba(0,255,159,0.5)', letterSpacing: '0.25em', marginBottom: 4 }}>TOTAL_ASSETS</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#00ff9f' }}>{Object.keys(port).length}</div>
                <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.25)', marginTop: 2 }}>tokens on Stacks mainnet</div>
              </div>
              {Object.entries(port).map(([token, bal]) => (
                <div key={token} style={{ padding: '14px 16px', borderRadius: 12, marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: `${TOKEN_COLOR[token] || '#fff'}06`, border: `1px solid ${TOKEN_COLOR[token] || '#fff'}18`, position: 'relative', overflow: 'hidden' }}>
                  <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: `${Math.min((bal / (totalPortfolio || 1)) * 100, 100)}%`, background: `${TOKEN_COLOR[token] || '#fff'}06` }} />
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 16 }}>
                <a href="https://app.velar.co" target="_blank" rel="noopener noreferrer" style={{ padding: '12px', borderRadius: 10, textAlign: 'center', textDecoration: 'none', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', fontSize: 9, letterSpacing: '0.15em', color: 'rgba(255,255,255,0.4)' }}>SWAP ON VELAR ↗</a>
                <a href="https://app.alexlab.co/swap" target="_blank" rel="noopener noreferrer" style={{ padding: '12px', borderRadius: 10, textAlign: 'center', textDecoration: 'none', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', fontSize: 9, letterSpacing: '0.15em', color: 'rgba(255,255,255,0.4)' }}>SWAP ON ALEX ↗</a>
              </div>
            </>
          )}
        </div>
      ...
      {tab === 'streak' && (
        <div style={{ flex: 1, padding: '16px', overflowY: 'auto' }}>
          <div style={{ padding: '24px', borderRadius: 16, textAlign: 'center', marginBottom: 14, background: checkedIn ? 'rgba(0,255,159,0.04)' : 'rgba(255,215,0,0.04)', border: checkedIn ? '1px solid rgba(0,255,159,0.2)' : '1px solid rgba(255,215,0,0.2)' }}>
            ...
            <p style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.2em', color: checkedIn ? '#00ff9f' : '#ffd700', marginBottom: 4 }}>{checkedIn ? 'CHECKED_IN_TODAY' : 'CHECK_IN_NOW'}</p>
            <p style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginBottom: 16 }}>{checkedIn ? `Come back tomorrow · Streak: ${streak.current} days` : '0.001 STX · Builds streak · Earns rewards'}</p>
            {!checkedIn && (
          ...
          <div style={{ padding: '16px', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            {[{ days: 7, bonus: '+0.002 STX', icon: '🥈' }, { days: 30, bonus: '+0.01 STX', icon: '🥇' }, { days: 100, bonus: '+0.05 STX', icon: '💎' }].map(b => (
              <div key={b.days} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span>{b.icon}</span><span style={{ fontSize: 10, color: streak.current >= b.days ? 'white' : 'rgba(255,255,255,0.3)' }}>{b.days}-DAY STREAK</span></div>
      ...
      {tab === 'withdraw' && (
        <div style={{ flex: 1, padding: '16px', overflowY: 'auto' }}>
          <div style={{ padding: '20px', borderRadius: 14, background: 'rgba(255,68,68,0.04)', border: '1px solid rgba(255,68,68,0.15)', marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', lineHeight: 1.5 }}>Withdraw treasury STX from check-in fees. Only the contract owner can call this.</p>
          </div>
          {!isConnected ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {[0.01, 0.05, 0.1, 0.5].map(v => (
                  <button key={v} onClick={() => setWithdrawAmt(String(v))} style={{ flex: 1, padding: '8px', borderRadius: 7, fontSize: 10, fontFamily: 'inherit', cursor: 'pointer', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.4)' }}>{v}</button>
                ))}
              </div>
              {withdrawErr && <div style={{ padding: '10px 12px', borderRadius: 8, background: 'rgba(255,68,68,0.07)', border: '1px solid rgba(255,68,68,0.2)', fontSize: 10, color: '#ff6666' }}>⚠ {withdrawErr}</div>}
              {withdrawTx && (
